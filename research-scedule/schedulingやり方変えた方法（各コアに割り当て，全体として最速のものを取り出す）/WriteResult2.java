        ...
        public void paintComponent(Graphics g){
            ...
            for(int i = 0; i < TaskGraph.total_tasks; i++){
                ...
                double length = 0;
                            ...
                            //スタートラインの描画
                            Line2D startline = new Line2D.Double(taskx, tasky -2, taskx, tasky + height +2);
                            g2.draw(startline);
                            
                            //タスクの処理時間を書きこむ
                            for(int n = 0; n < TaskGraph.task[i].executeFrequencyStep.length; n++){
                                if(TaskGraph.task[i].executeFrequencyStep[n] != 0.0){
                                    if(counter == (int)length)break;
                                    Rectangle2D shape2 = new Rectangle2D.Double(taskx , tasky, 0.2 , height);
                                    taskx+=0.2;
                                    //System.out.print(" "+TaskGraph.task[i].executeFrequencyStep[n]);
                                    if(TaskGraph.task[i].executeFrequencyStep[n] == 2.7)g2.setColor(Color.RED);
                                    else if(TaskGraph.task[i].executeFrequencyStep[n] == 2.5)g2.setColor(Color.GREEN);
                                    else if(TaskGraph.task[i].executeFrequencyStep[n] == 2.0)g2.setColor(Color.GRAY);
                                    else if(TaskGraph.task[i].executeFrequencyStep[n] == 1.9)g2.setColor(Color.CYAN);
                                    else if(TaskGraph.task[i].executeFrequencyStep[n] == 1.3)g2.setColor(Color.CYAN);
                                    g2.fill(shape2);
                                    counter++;
                                }
                            }
                            ...
                            Line2D endline = new Line2D.Double(taskx, tasky -10, taskx, tasky+ 1 + height +10);
                            g2.draw(endline);
                            g2.setColor(Color.cyan);
                            taskx = TaskGraph.task[i].getFinish_time() * up + 100;
                            Line2D endline2 = new Line2D.Double(taskx, tasky -5, taskx, tasky + 1 + height +5);
                            g2.draw(endline2);
