 ...
 * @author 1151118
 */
public class WriteResult3 extends JFrame{
    private int Max = 255;
    JPanel panel;
    JLabel label;
    
    public WriteResult3(String str, Proclist Plist, DAG TaskGraph){
        panel = new JPanel();
            ...
            for(int i = 0; i < TaskGraph.total_tasks; i++){
                ...
                float length = 0;
                            ...
                            if(!str.equalsIgnoreCase("n0")){
                                ...
                                String ftime = String.format("%.3f", TaskGraph.task[i].finish_time.get(TaskGraph.task[i].finish_time.size() - 1));
                            ...
                            //タスクの処理時間を書きこむ
                            for(int n = 0; n < TaskGraph.task[i].ExFrequency.size(); n++){
                                //if(counter == (int)length)break;
                                Rectangle2D shape2 = new Rectangle2D.Float(taskx , tasky, TaskGraph.task[i].working_time.get(n) * 10 , height);
                                taskx += TaskGraph.task[i].working_time.get(n) * 10;
                                //System.out.print(" "+TaskGraph.task[i].executeFrequencyStep[n]);
                                if(TaskGraph.task[i].ExFrequency.get(n) == (float)3.7)g2.setColor(Color.RED);
                                else if(TaskGraph.task[i].ExFrequency.get(n) == (float)3.5)g2.setColor(Color.GREEN);
                                else if(TaskGraph.task[i].ExFrequency.get(n) == (float)3.3)g2.setColor(Color.BLUE);
                                else if(TaskGraph.task[i].ExFrequency.get(n) == (float)3.1)g2.setColor(Color.LIGHT_GRAY);
                                else if(TaskGraph.task[i].ExFrequency.get(n) == (float)3.0)g2.setColor(Color.CYAN);
                                else if(TaskGraph.task[i].ExFrequency.get(n) == (float)2.5)g2.setColor(Color.CYAN);
                                else if(TaskGraph.task[i].ExFrequency.get(n) == (float)100)g2.setColor(Color.ORANGE);
