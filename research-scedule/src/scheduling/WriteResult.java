...
public class WriteResult extends JFrame{
    ...
    public WriteResult(String str, Proclist Plist, DAG TaskGraph){
        panel = new JPanel();
    ...
    class MyRect extends JPanel{
        ...
        Proclist Plist;
        float stime , ftime;
        private int initialx = 10; // 描画開始初期位置X座標
        private int initialy = 10; // 描画開始初期位置Y座標
        private int up = 10;       // 描画の拡大倍率
        private float taskx;       // 各処理時間描画の開始位置X
        private float tasky;       // 各処理時間描画の開始位置Y
        private int width;
                ...
                float length = 0;
                            ...
                            for(int n = 0; n < TaskGraph.task[i].executeFrequencyStep.size(); n++){
                                if(TaskGraph.task[i].executeFrequencyStep.get(n) != 0.0){
                                    ...
                                    Rectangle2D shape2 = new Rectangle2D.Float(taskx , tasky, (float)5 , height);
                                    taskx+=5;
